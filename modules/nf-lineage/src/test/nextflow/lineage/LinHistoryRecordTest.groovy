...
class LinHistoryRecordTest extends Specification {
    ...
    def "LinHistoryRecord parse should handle 5-column record"() {
        given:
        def timestamp = new Date()
        def formattedTimestamp = LinHistoryRecord.TIMESTAMP_FMT.format(timestamp)
        def line = "${formattedTimestamp}\trun-1\t${UUID.randomUUID()}\tlid://123\tlid://456"
    ...
    def "LinHistoryRecord toString should produce tab-separated format"() {
        ...
        def record = new LinHistoryRecord(new Date(), "TestRun", sessionId, "lid://123", "lid://456")
